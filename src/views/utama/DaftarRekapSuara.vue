<template>
        ...
        <b-card no-body class="card">
          <div class="card-header d-flex justify-content-between flex-wrap align-items-center">
            ...
            <div class="d-flex align-items-center gap-3">
              <button v-b-modal.modalDropZone class="text-center btn btn-primary d-flex gap-2">
    ...
    <b-modal id="modalDropZone" hide-backdrop hide-footer centered size="xl" title="Bulk Upload">
        <div>
            <div v-bind="getRootProps()" class="dropzone d-flex flex-column justify-content-between">
                <input v-bind="getInputProps()" />
                <p class="m-0" v-if="isDragActive">Drop file disini ...</p>
                <p class="m-0" v-else>Drag & drop beberapa file disini, atau klik untuk memilih file</p>
                ...
                <div v-if="filePreviews.length > 0" class="file-previews">
                    ...
                    <img :src="preview" class="border-success border border-2"/>
                    </div>
                </div>
            </div>
        </div>
  ...
  export default {
        ...
        const saveFiles = async (files) => {
            try {
                ...
                for (let x = 0; x < files.length; x++) {
                    formData.append(`image_uploads[${x}]`, files[x]);
                }

                const response = await axios.post(`${process.env.VUE_APP_BACKEND_API}/api/v1/voter-caleg-recap/store`, formData, {
                    headers: {
    ...
    methods: {
      ...
      cariData: debounce(async function (showAll, page) {      
        ...
        const response = await axios.get(`${process.env.VUE_APP_BACKEND_API}/api/v1/voter-caleg-recap/list${queryParam ?? ''}`, withHeader);
